...
// scroll-margin-top は固定 TabBar の高さ + 余白を見越して styles.css 側で設定。
//
// hunks prop:
...
import { useLazyHighlight } from './useLazyHighlight.ts'
...
// React.memo で props 浅比較。GroupSection 同様、上位 state の軽微な変化で全 file が
// 再 render される事態を防ぐ。lineCommentHandlers は useLineComments の useMemo で
// 安定化済み、reviewed / comment はプリミティブ値で渡ってくるので比較は cheap。
export const FileBlock = memo(function FileBlock({
  ...
  // article 全体を観察。viewport 付近に来たらこの file 配下の行を Shiki highlight 対象に昇格させる。
  // header 単体ではなく article 全体を対象にする理由: 「body は画面内だが header が画面外」という
  // スクロール位置でも highlight を維持したいため。一度可視 → 以後 true 固定なので、最初に
  // どちらか入ればその後はずっと highlight 状態。
  const articleRef = useRef<HTMLElement>(null)
  const highlight = useLazyHighlight(articleRef)

  return (
    <article ref={articleRef} className="file-block file" data-path={file.path}>
