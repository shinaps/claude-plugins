...
import type { ClientPayload, Hunk, ParsedFile, SideBySideRow } from '@zeus/review-diff-shared'

// ---------- row helpers ----------

function ctxRow(leftLn: number, rightLn: number, raw: string): SideBySideRow {
  return {
    left: { type: 'context', line: leftLn, raw },
    right: { type: 'context', line: rightLn, raw },
  }
}

function addRow(rightLn: number, raw: string): SideBySideRow {
  return {
    left: { type: 'empty', raw: '' },
    right: { type: 'addition', line: rightLn, raw },
  }
}

function delRow(leftLn: number, raw: string): SideBySideRow {
  return {
    left: { type: 'deletion', line: leftLn, raw },
    right: { type: 'empty', raw: '' },
  }
}
