...
3. ファイルサイズや行数で「重そう」「長そう」な候補を絞る
4. 候補上位を `Read` して中身を確認
5. **最も改善効果が高い 1 件** を選択 (複数候補で迷ったら、行数 / ネスト深さ / 重複の程度 / 命名の悪さ を総合判断)

#### 改善観点 (優先順)

1. **長すぎる関数 / メソッド** (50 行超目安、ネスト 3 段以上を含む)
2. **重複コード** (同じパターンが 3 箇所以上)
3. **命名の不明瞭さ** (1-2 文字変数、`data` / `temp` / `obj` / `handle` 等の曖昧名)
4. **責務過多のクラス / モジュール** (1 ファイル 300 行超、複数の関心事が同居)
5. **マジックナンバー / マジックストリング**
6. **不必要な if-else ネスト** (early return で平坦化できる)
7. **型の any 多用** (TypeScript の場合)
8. **コメントと実装の乖離** (古いコメント)
